        ...
        <div class="app-search">
          <el-form :inline="true" size="small" :model="search" class="demo-form-inline" ref="search">
            <el-form-item label="任务名称">
              <el-input v-model="search.taskName" placeholder="任务名称" clearable></el-input>
            </el-form-item>
            <el-form-item label="任务状态">
              <el-select v-model="search.taskStatus" clearable>
                <el-option label="执行失败" value="0"/>
                <el-option label="等待执行" value="1"/>
                <el-option label="正在执行" value="2"/>
                <el-option label="执行成功" value="3"/>
                <el-option label="取消执行" value="4"/>
              </el-select>
            </el-form-item>
            <el-form-item>
        ...
        >
          ...
          <el-table-column label="操作" width="165" fixed="right">
            <template slot-scope="scope">
              ...
              <el-button size="mini" type="text" @click="handleShow(scope.row.id, 3)">记录</el-button>
              <el-button v-if="scope.row.taskStatus==1" size="mini" type="text" @click="closeActFn(scope.row.id)">取消</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-pagination
...
import Http from '@/api/setting/task'
...
export default {
  ...
  methods: {
    initTable() {
      Http.queryTask(this.search).then((response) => {
    ...
    handleShow(id, opt) {
      if(opt==3){
        this.$emit('viewRecord', id)
        return
      }
      this.$router.push({path: '/setting/taskModule/taskInfo', query: {id: id, opt: opt}})
    },
    closeActFn(id) {
      this.$confirm(
          '确认取消吗?',
      ...
      .then(() => {
        Http.cancelSendEmailTem(id).then((res) => {
          if (res.code === 200) {
