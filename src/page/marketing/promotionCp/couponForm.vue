        ...
        <el-form :model="dataForm" ref="dataForm" label-width="140px" :rules="rules" :disabled="opt==2" >
          ...
          <el-form-item prop="promotionType" label="活动类型">
            <el-radio-group v-model="dataForm.promotionType">
              ...
              <el-radio :label="2">满额折</el-radio>
              <el-radio :label="3">满量减</el-radio>
              <el-radio :label="4">满量折</el-radio>
            </el-radio-group>
          </el-form-item>
          ...
          <el-form-item prop="discountType" label="优惠设置">
            <el-radio-group v-model="dataForm.discountType">
              <el-radio :label="1">阶梯满{{promotionTypeStr}}</el-radio>
              <el-radio :label="2">
                循环满{{promotionTypeStr}}
                <el-form-item v-if="dataForm.discountType==2" prop="discountLimit" label="" style="display: inline-block;margin-left:15px;">
          ...
          <el-form-item label="" v-if="dataForm.discountType">
            <el-table ref="multipleTable"
                      :data="dataForm.discountType == 1 ? discountSettingDTOS1 : discountSettingDTOS2"
                      row-key="index"
                      border
                      size="small">
              <el-table-column
              ...
              <el-table-column
                  label="优惠内容">
                <template slot-scope="{row}">
                  {{promotionTypeStr3}} <el-input-number v-model="row.discountContent" size="small" :controls="false" style="width: 80px;" /> {{promotionTypeStr4}}
                </template>
              </el-table-column>
<!--              <el-table-column-->
<!--                  label="赠送商品">-->
...
export default {
  ...
  data() {
    return {
      ...
      openDialog: false,
      istrue: true,
      discountSettingDTOS1: [deepClone(discountSetting)],
      discountSettingDTOS2: [deepClone(discountSetting)],
      row: null,
  ...
  watch:{
    $route(val){
      if((this.$route.path.indexOf('promotionCp/couponForm')>0 && ((this.$route.query.id!=this.dataForm.id) || this.opt != this.$route.query.opt))){
        console.log('promotionCp/couponForm刷新')
        this.init()
  ...
  computed: {
    promotionTypeStr() {
      let str1 = '减'
      if(this.dataForm.promotionType == 1 || this.dataForm.promotionType == 3){
        str1 = '减'
      }else{
        str1 = '折'
      }
    ...
    promotionTypeStr2() {
      let str1 = '元'
      if(this.dataForm.promotionType == 1 || this.dataForm.promotionType == 2){
        str1 = '元'
      }else{
        str1 = '件'
      }
  ...
  methods:{
    ...
    init(){
      this.steps = 1
      this.discountSettingDTOS1 = [deepClone(discountSetting)]
      this.discountSettingDTOS2 = [deepClone(discountSetting)]
      if(!this.$route.query.id){
        this.resetData()
        this.$nextTick(()=>{
      ...
      getById(this.$route.query.id).then((res)=>{
        ...
        if(res.code === 200){
          ...
          console.log(discountSettingDTOSList)
          console.log(this.discountSettingDTOS1)
          console.log(this.discountSettingDTOS2)
    ...
    submitForm(form){
      ...
      console.log(this.discountSettingDTOS1, this.discountSettingDTOS2)
      this.$refs[form].validate((valid) => {
        ...
        const discountSettingDTOSList = deepClone(this[`discountSettingDTOS${this.dataForm.discountType}`])

        discountSettingDTOSList.forEach((v)=>{
          v.discountType = this.dataForm.discountType
          v.discountLimit = this.dataForm.discountLimit
          if(!v.discountThreshold || !v.discountContent){
            this.istrue = 1
          }
          if(v.discountThreshold <= v.discountContent){
            this.istrue = 2
        ...
        if(this.istrue!==0){
          if (this.promotionTypeStr === '减') {
            this.$baseMessage((this.istrue==1?'优惠设置数据不完整':'优惠设置满金额必须大于减金额'), 'error')
            return
          } else {
            // 折扣
            if(this.istrue==1){
        ...
        if(discountSettingDTOSList && discountSettingDTOSList.length>1){
          let flag = true
          discountSettingDTOSList.reduce((pre,cur)=>{
            if(pre.discountThreshold>cur.discountThreshold) flag = false
            return cur
          },0)
          if(!flag){
            this.$baseMessage('优惠设置阶梯值必须大于上一阶梯', 'error')
            return
          }
        }
        ...
        params.discountSettingDTOS = discountSettingDTOSList
    ...
    closeNode(){
      this.$router.push('/marketing/promotionCp/couponList')
      this.$store.dispatch('permission/closeTagsByPath', 'marketing/promotionCp/couponForm')
    },
    selectOpen(row) {
      this.row = row
    ...
    addDiscount() {
      ...
      // console.log(dis)
      this.discountSettingDTOS1.push(dis)
    },
    deleteDis(row, type) {
