...
<script>
import {closeAct, querylist} from '@/api/marketing/promotion'
...
export default {
  ...
  data() {
    return {
      pathUrl: 'marketing/promotionCp/couponList',
      dialogVisible: false,
      title: '',
      data: [],
      column: [
        ...
        {label: '适用商品', prop: 'promotionScope'},
  ...
  methods: {
    initTable() {
      ...
      if(!params.promotionType){
        ...
        params.promotionTypeList = [1,2,3,4]
      }
      querylist(params).then((response) => {
        if (response.code === 200 && response.data.records && response.data.records.length>0) {
          ...
          //   v.desc = !v.orderFullAmount ? '无门槛' : (`满${v.orderFullAmount}元，${v.promotionAmountStr}`)
            v.promotionScope = v.promotionScope === 0 ? '全部' : (v.promotionScope == 1 ? '指定商品可用' : '指定品类可用')
    ...
    handleShow(id, opt) {
      ...
      this.$router.push({path: '/marketing/promotionCp/couponForm',query: {id: id, opt: opt}})
    },
    changeDate() {
