  ...
  <div>
    <el-row>
      <div class="table-body">
        ...
        <el-col :span="24">
          <div style="text-align:right;height:40px;" v-if="checkMore"><el-button type="primary" size="small" @click="addgoods">选择</el-button></div>
          <el-table
              ref="multipleTable"
              :data="data"
              border
          ...
          >
            ...
            <el-table-column label="操作" prop="skuCode" width="80" v-if="!checkMore">
              <template slot-scope="scope">
...
export default {
  ...
  methods:{
    selectEvent(row,index){
      // console.log(row)
      if(this.multioleSelectionData && this.multioleSelectionData.length>0 && this.multioleSelectionData.indexOf(row.skuCode)>-1){
        row.checked = true
        return false
      }else{
        return true
